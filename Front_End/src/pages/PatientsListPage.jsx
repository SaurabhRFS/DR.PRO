...
// Added Hash icon
import { Users, UserPlus, Search, ArrowUpDown, Filter, CalendarMinus, DollarSign, Loader2, AlertTriangle, Hash } from 'lucide-react';
...
const PatientsListPage = () => {
  ...
  const filteredAndSortedPatients = useMemo(() => {
    let filtered = processedPatients.filter(patient =>
      ...
      // Search in 'email' field which now holds the ID
                    ...
                    <Card className="h-full hover:shadow-xl transition-all duration-300 ease-out group dark:bg-slate-800 dark:hover:bg-slate-700/80 dark:border-slate-700">
                      ...
                      <CardHeader className="flex flex-row items-center gap-6 p-6">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <CardDescription className="text-base mt-1 dark:text-slate-400 flex flex-col gap-1">
                            {/* Phone */}
                            <span className="truncate">{patient.phone}</span>
                            
                            {/* Display ID below Phone */}
                            {patient.email ? (
                                <span className="flex items-center gap-1 text-sm font-bold text-primary">
                                    <Hash className="h-3 w-3" /> ID: {patient.email}
                                </span>
                            ) : (
                                <span className="flex items-center gap-1 text-sm italic text-muted-foreground">
                                    No ID
                                </span>
                            )}
                          </CardDescription>
