...
const getFullUrl = (path) => {
  if (!path) return null;
  // If it's already a full link (Cloudinary/S3), leave it alone
  if (path.startsWith('http')) return path;
  // Otherwise, attach your Java Server URL
  // If path starts with /uploads, just prepend domain. If just filename, add /uploads/
...
const AppointmentCard = ({ appointment, patientName, onEdit, onDelete, onStatusChange, index }) => {
  ...
  // Combine legacy single file and new multiple files
...
// The backend might be sending 'prescriptionFileName', 'fileName', or 'filePath'
const rawFileName = appointment.prescriptionUrl 
                 || appointment.prescriptionFileName 
                 || appointment.fileName 
                 || appointment.prescriptionPath;

const prescriptionFullUrl = getFullUrl(rawFileName);

// DEBUGGING: Open your browser console (F12) to see exactly what the database is sending
    ...
    >
      <Card className="hover:shadow-lg transition-shadow border-l-4 h-full flex flex-col" style={{ borderColor: cardBorderColor() }}>
        ...
        <CardContent className="p-4 pt-0 grid grid-cols-2 sm:grid-cols-3 gap-2 text-sm flex-grow">
          ...
          {/* --- Images Section (UPDATED TO USE FULL URLS) --- */}
